    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: '"Figtree", "Inter", sans-serif' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #E5E7EB', padding: '32px 24px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: 32, fontWeight: 800, color: '#201F23', margin: '0 0 8px' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '32px auto', padding: '0 24px' }}>
        {products.length === 0 ? (
          <div style={{ background: '#fff', padding: 48, borderRadius: 12, textAlign: 'center', border: '1px solid #E5E7EB' }}>
            <h2 style={{ fontSize: 20, color: '#201F23', marginBottom: 12 }}>No products selected for comparison</h2>
            <Link to="/" style={{ color: '#5A39A2', fontWeight: 600, textDecoration: 'none' }}>Back to Categories</Link>
          </div>
        ) : (
          <div style={{
            display: 'grid',
            ...
            {products.map((prod) => (
              <div key={prod.id} style={{
                ...
                <div style={{ textAlign: 'center', paddingBottom: 20, borderBottom: '1px solid #F3F4F6', marginBottom: 20 }}>
                  ...
                  <h3 style={{ fontSize: 20, fontWeight: 700, color: '#201F23', margin: '0 0 4px' }}>{prod.name}</h3>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, marginTop: 8 }}>
                    {renderStars(prod.rating)}
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#201F23', marginLeft: 4 }}>{prod.rating}/5</span>
                  </div>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 13, color: '#201F23', lineHeight: 1.6, margin: 0 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      <span style={{ fontSize: 12, padding: '4px 10px', background: '#F3F4F6', borderRadius: 20, color: '#201F23', fontWeight: 600 }}>
                        {prod.solutionType || 'Best-of-Breed'}
                      </span>
                      {prod.aiBadge && (
                        <span style={{ fontSize: 12, padding: '4px 10px', background: '#EDE9FE', borderRadius: 20, color: '#5A39A2', fontWeight: 600 }}>
                          {prod.aiBadge}
                        </span>
                      )}
                    </div>
                ...
                <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid #F3F4F6' }}>
                  <Link to={`/product/${prod.id}`} style={{
                    display: 'block', textAlign: 'center', background: '#5A39A2', color: '#fff',
